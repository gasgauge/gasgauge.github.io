...
contract MainframeStake is Ownable, StakeInterface {
  ...
  address payable _owner;  //added by 'Jolly'
  ...
  */
  ...
  function stake(address whitelistAddress) external returns (bool success) {
    require(whitelist[whitelistAddress].stakerAddress == address(0)); //0x0
  ...
  */
  ...
  function deposit(address fromAddress, uint256 depositAmount) private returns (bool success) {
    token.transferFrom(fromAddress, address(this), depositAmount);
  ...
  */
  ...
  function refundBalances(address[] calldata addresses) external onlyOwner {
  ...
  function emergencyERC20Drain(ERC20 _token) external onlyOwner {
    // owner can drain tokens that are sent here by mistake.----Jolly asked " but where is the "owner" here from?"
    uint256 drainAmount;
    if (address(_token) == address(token)) {
      drainAmount = _token.balanceOf(address(this)).sub(totalDepositBalance);
    } else {
      drainAmount = _token.balanceOf(address(this));
    }
    _token.transfer(owner(), drainAmount);  //there is  a function named owner in Ownable.sol----specification added by Jolly
  }

  function destroy() external onlyOwner {
    require(token.balanceOf(address(this)) == 0);
	_owner=owner();
    selfdestruct(_owner);  //change owner to be owner()---specification added by Jolly
  }
