...
contract PoolReward is LPTokenWrapper {
    ...
    uint256 public constant oneday = 15 days;

    uint256 public initreward = 0;
    uint256 public starttime = 0; //09/11/2020 @ 10:00 (UTC)
    ...
    modifier updateReward(address account) {
		if(lastTimeRewardApplicable() > starttime && lastUpdateTime < endtime){
...
contract RewardPoolFactory{
    ...
    function createRewardPool(address _lptoken,address _rewardtoken,uint256 _initreward,uint256 _starttime,uint256 _duration) public returns (PoolReward addr){
        addr = new PoolReward(_lptoken,_rewardtoken,_initreward,_starttime,_duration);
    }
