...
contract STEST22Token is StandardToken {
    ...
    event Mint(address indexed from, uint tokens);// address(this) notifies clients about the amount minted

    event Burn(address indexed from, uint tokens);// address(this) notifies clients about the amount burnt

    

    address public owner = address(0);//Admin actions

    address public minter = address(0);//Minter tokens

 

    constructor(address _owner, address _minter) public payable {
    ...
    function mintTokens(address _target, uint _amount) external returns (bool) {
        ...
        emit Transfer(address(this), _target, amount);
    ...
    function decreaseTokens(address _target, uint _amount) external returns (bool) {
        ...
        emit Transfer(_target, address(0), amount);
