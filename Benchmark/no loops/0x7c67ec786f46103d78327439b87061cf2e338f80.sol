...
contract VerifyYourContractsAssholes is ERC20 {
    ...
    function transfer(address recipient, uint256 amount) public   limitEarlyBuy (amount) returns (bool) {
        _transfer(_msgSender(), recipient, amount);
        return true;
    }

    function transferFrom(address sender, address recipient, uint256 amount) public   limitEarlyBuy (amount) returns (bool) {
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: transfer amount exceeds allowance"));
        return true;
    }
